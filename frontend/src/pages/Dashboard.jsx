...
function Dashboard() {
  ...
  // ── Only change: wrap with PageLayout ──
  return (
    <PageLayout>
      <div className="space-y-8 w-full pb-12 min-w-0 overflow-x-hidden p-8">
            ...
            <div className="hidden sm:block text-right leading-tight">
              {loading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-400 font-medium mt-0.5 group-hover:text-indigo-600 transition-colors">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div className="space-y-4">
              ...
              <p className="text-slate-300 text-sm max-w-xl leading-relaxed font-medium">
                Maintain daily consistency across coding, quizzes, and interview preparation.
              </p>
              <div className="flex flex-wrap gap-4 pt-2">
                {[
                  ...
                  <div key={s.label}
                    className="bg-white/[0.04] border border-white/5 backdrop-blur-md rounded-xl px-5 py-3 min-w-[120px]">
                    <p className="text-slate-400 text-[10px] font-bold tracking-[0.12em] uppercase">
                      {s.label}
                    </p>
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => navigate("/quiz")}
                className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold text-sm hover:scale-[1.02] transition-transform">
                Start Timed Quiz
              </button>
              <button onClick={() => navigate("/coding")}
                className="px-6 py-3.5 rounded-xl border border-slate-700/60 bg-slate-900/40 text-slate-200 font-bold text-sm hover:bg-slate-900/80 transition-colors">
                Solve Challenges
              </button>
            </div>
          </div>
        ...
        {/* ACTIVITY + WEAK AREAS — your original */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-stretch">

          <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm h-full">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
              <h3 className="text-base font-bold text-slate-900 tracking-tight">
                Recent Execution Streams
              </h3>
              ...
              <div className="space-y-2.5">
                {recentActivity.length === 0 ? (
                  <div className="py-10 text-center">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Start practicing to see history</p>
                  </div>
                ) : recentActivity.map((a, i) => (
                  <div key={i} onClick={() => navigate(a.path)}
                    className="group flex items-center justify-between p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:border-indigo-200">
                    ...
                    <span className="text-xs font-extrabold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-md shrink-0">
                      {a.score}
                    </span>
                  </div>
                ))}
          ...
          <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm h-full">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center flex flex-col items-center justify-center border border-emerald-100 bg-emerald-50/20 rounded-xl">
                ...
                <h4 className="text-sm font-bold text-emerald-800 mt-2">Zero Platform Vulnerabilities</h4>
                <p className="text-xs text-slate-400 mt-0.5">All modules performing above baseline.</p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {progress.weakAreas.slice(0, 4).map((w, idx) => (
                  <div key={idx}
                    className="p-3.5 bg-rose-50/30 border border-rose-100 rounded-xl flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <AlertTriangle size={14} className="text-rose-500 shrink-0" />
                        <h4 className="font-bold text-xs text-slate-800 truncate">{w.category}</h4>
                      </div>
                      <p className="text-[10px] text-slate-400 font-bold tracking-wider uppercase mt-1">
                        {w.module}
                      </p>
                    </div>
                    <span className="text-xs font-black text-rose-600 bg-rose-50 border border-rose-100 px-2.5 py-1 rounded-md">
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              Instantly fast-forward your preparation workflow.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
            {modules.map((mod, idx) => (
              <Link key={idx} to={mod.path}
                className="group relative overflow-hidden bg-white border border-slate-200/60 p-5 rounded-xl hover:shadow-md hover:border-slate-300 transition-all duration-200 flex flex-col items-center justify-center min-h-[170px]">
                <div className={`absolute -right-6 -bottom-6 w-20 h-20 bg-gradient-to-br ${mod.gradient} opacity-[0.03] rounded-full`} />
                <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${mod.gradient} text-white flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform`}>
                  {mod.icon}
                </div>
                <div className="flex-1 flex items-center justify-center">
                  <h4 className="text-[17px] font-black text-slate-800 text-center leading-snug tracking-tight group-hover:text-indigo-600 transition-colors">
                    {mod.title}
                  </h4>
                </div>
              </Link>
